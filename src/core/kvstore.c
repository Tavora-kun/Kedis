...
int is_read_command(const char* command) {
    ...
    /* 查询类命令：只读不写的命令 */
    if (strcmp(command, "GET") == 0 || strcmp(command, "RGET") == 0 ||
            strcmp(command, "HGET") == 0 || strcmp(command, "AGET") == 0 ||
            strcmp(command, "SGET") == 0 ||
            /* 存在性检查：只读 */
            strcmp(command, "EXIST") == 0 || strcmp(command, "REXIST") == 0 ||
            strcmp(command, "HEXIST") == 0 || strcmp(command, "AEXIST") == 0 ||
            strcmp(command, "SEXIST") == 0 ||
            /* 持久化命令：不修改 KV 数据 */
            strcmp(command, "SAVE") == 0 || strcmp(command, "BGSAVE") == 0 ||
            /* 同步触发命令：控制命令，非数据修改 */
