...
# -*- coding: utf-8 -*-
"""
SSET增量复制配置对比图表生成脚本 - 单图版本
根据实际memtier_benchmark JSON结构解析数据
每个指标生成单独的图表，字体清晰不重叠

使用方法:
    python3 gen_repl_charts.py <结果目录>

    示例:
    python3 gen_repl_charts.py ./repl_sset_benchmark_results
"""
...
def create_overhead_chart(output_dir, configs):
    ...
    if 'NoMirror' not in configs:
        print("  ! 跳过性能损耗图（无NoMirror数据）")
        return None

    baseline_qps = configs['NoMirror']['ops_sec']

    # 只包含有损耗的配置
    overhead_configs = {k: v for k, v in configs.items() if k != 'NoMirror'}
    ...
    ax.set_title('Performance Overhead vs No Mirror', fontsize=16, fontweight='bold', pad=20)
...
def create_summary_table(output_dir, configs):
    ...
    ax.set_title('Replication Performance Summary Table', fontsize=16, fontweight='bold', pad=20)
...
def main():
    if len(sys.argv) < 2:
        print("用法: python3 gen_repl_charts.py <结果目录>")
        print("示例: python3 gen_repl_charts.py ./repl_sset_benchmark_results")
        sys.exit(1)
