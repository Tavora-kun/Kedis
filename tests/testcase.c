...
void print_usage(const char* prog) {
    ...
    printf("  ksf         - KSF persistence test\n");
    printf("  aof         - AOF persistence test\n");
    printf("  load-ksf    - KSF load test (verify KSF snapshot)\n");
    printf("  load-aof    - AOF load test (verify AOF log)\n");
    printf("\nOptions:\n");
    ...
    printf("  %s 127.0.0.1 8888 ksf\n", prog);
    printf("  %s 127.0.0.1 8888 aof\n", prog);
    printf("\nPersistence Testing:\n");
    printf("  Use test_persistence.sh script for automated persistence testing\n");
    printf("  ./test_persistence.sh ksf   - Test KSF persistence\n");
    printf("  ./test_persistence.sh aof   - Test AOF persistence\n");
    printf("  ./test_persistence.sh all   - Test both KSF and AOF\n");
}
...
void run_tests(int connfd) {
    ...
    for (int i = 0; i < g_engine_count; i++) {
        ...
        // KSF 持久化测试
        if (strcmp(g_config.mode, "ksf") == 0) {
            test_ksf_persistence(connfd, engine);
            continue;
        }
        
        // AOF 持久化测试
        if (strcmp(g_config.mode, "aof") == 0) {
            test_aof_persistence(connfd, engine);
            continue;
        }
