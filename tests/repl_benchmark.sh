...
# =============================================================================
# SSET增量复制配置测试脚本 - 交互式版本
# 功能: 让用户选择增量复制方案，执行100万次SSET测试，生成带标签的结果
...
# 输出目录
OUTPUT_DIR="./repl_sset_benchmark_results"
mkdir -p "${OUTPUT_DIR}"
...
while true; do
    ...
    case $choice in
        1)
            CONFIG_NAME="NoMirror"
            CONFIG_LABEL="REPL=NONE"
            break
            ;;
        2)
            CONFIG_NAME="TC_Ingress"
            CONFIG_LABEL="REPL=TC_INGRESS"
            break
            ;;
        3)
            CONFIG_NAME="XDP"
            CONFIG_LABEL="REPL=XDP"
            break
            ;;
        4)
            CONFIG_NAME="IORecv"
...
REPL: $(echo ${CONFIG_LABEL} | grep -o 'REPL=[^,]*' | cut -d= -f2)
HOST: ${HOST}
PORT: ${PORT}
THREADS: ${THREADS}
