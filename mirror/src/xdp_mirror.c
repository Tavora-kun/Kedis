...
static unsigned int hash_5tuple(__u32 sip, __u32 dip, __u16 sport,
                                __u16 dport) {
...
// [新增] 查找或创建流缓冲区的辅助函数
static struct reassembly_buffer* get_or_create_flow(__u32 sip, __u32 dip,
  ...
  // 创建新节点
  struct reassembly_buffer* new_buf =
      calloc(1, sizeof(struct reassembly_buffer));
  if (!new_buf) {
    perror("calloc");
    return NULL;
  }
...
static void remove_flow(struct reassembly_buffer* target) {
  ...
  unsigned int idx = hash_5tuple(target->src_ip, target->dst_ip,
                                 target->src_port, target->dst_port);
...
static void flush_buffer(struct reassembly_buffer* buf) {
  ...
  if (buf->received_len != buf->total_len) {
    ...
    // fprintf(stderr, "[Drop] Flow incomplete: expected %u, got %u. Dropping to
  ...
  if (slave_fd >= 0) {
    ...
    while (sent < buf->received_len) {
      ssize_t n = send(slave_fd, buf->data + sent, buf->received_len - sent, 0);
      if (n < 0) {
...
static int handle_event(void* ctx, void* data, size_t data_sz) {
  ...
  if (e->type == EVENT_HEADER) {
    ...
    if (buf->active) {
      ...
      buf =
          get_or_create_flow(e->src_ip, e->dst_ip, e->src_port, e->dst_port, 1);
    }
    ...
    buf->received_len = 0;
    // mirror_logDebug("初始化: buf->total_len == payload_len: %u",
    // buf->total_len);
...
int main(int argc, char** argv) {
  ...
  // Step 4: 打开并加载 BPF 程序
  skel = xdp_mirror_bpf__open_and_load();
  if (!skel) {
  ...
  prog_fd = bpf_program__fd(skel->progs.xdp_mirror_forward);
  if (prog_fd < 0) {
  ...
  // Step 7: 主循环 - 显示统计信息
  printf("\nXDP mirror is running. Press Ctrl+C to exit.\n");
